/****************************************************************************
** Meta object code from reading C++ file 'exceptionparams.h'
**
** Created: Sun Dec 1 18:46:51 2019
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../src/ParaConfig/configure_params/RemoteParams/ExceptionParams/exceptionparams.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'exceptionparams.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_ExceptionParams[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      23,   17,   16,   16, 0x0a,
      39,   16,   16,   16, 0x0a,
      55,   16,   16,   16, 0x0a,
      74,   68,   16,   16, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_ExceptionParams[] = {
    "ExceptionParams\0\0index\0changeType(int)\0"
    "RefreshParams()\0SaveParams()\0state\0"
    "changeState(int)\0"
};

const QMetaObject ExceptionParams::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_ExceptionParams,
      qt_meta_data_ExceptionParams, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &ExceptionParams::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *ExceptionParams::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *ExceptionParams::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_ExceptionParams))
        return static_cast<void*>(const_cast< ExceptionParams*>(this));
    return QWidget::qt_metacast(_clname);
}

int ExceptionParams::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: changeType((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: RefreshParams(); break;
        case 2: SaveParams(); break;
        case 3: changeState((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
